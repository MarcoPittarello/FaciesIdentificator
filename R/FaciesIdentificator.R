...
#' @return Matrici con le facies più simili a ciascun rilievo da identificare, ordinate per indice di somiglianza (Bray o Jaccard) e con indicazione dell'abbondanza delle specie nelle facies di riferimento.
#' @examples ...
#' @import vegan
#' @export
...
FaciesIdentificator = function(
  ...
  tbd10 <- Reduce(
    function(x, y) merge(x, y, by = "specie", all = TRUE),
    tdf.list10
  )
  tbd10[is.na(tbd10)] <- 0
  ...
  tenSpe.Ref <- do.call(
    ...
    lapply(2:ncol(ref), function(i) {
      ...
      ref.sel$spe.abb <- paste(
        make.cepnames(ref.sel[, 1]),
        round(ref.sel[, 2], 1),
        sep = " "
      )
      colnames(ref.sel)[2] <- "abbondanza"
      ref.sel <- subset(ref.sel, abbondanza > 0)
      df.ref.sel <- data.frame(
        Facies = name.ref.sel,
  ...
  # cep names
  cep <- data.frame(specie = data$specie, cep = make.cepnames(data$specie))
  ...
  risultato <- lapply(2:ncol(data), function(i) {
    ...
    # bray
    brayDist <- data.frame(as.matrix(vegdist(
      t.mrgd,
      method = "bray",
      upper = T
    )))
    ...
    # Jaccard
    Jacc <- data.frame(as.matrix(vegdist(
      t.mrgd,
      binary = T,
      method = "jaccard",
      upper = T
    ...
    if (soglia == T) {
      ...
      output <- list(
        info.rilievo = output.info.rilievo,
  ...
  ris.bray <- cbind(
    data.frame(rilievo = rownames(ris.bray)),
    data.frame(ris.bray)
  )
  ...
  ris.Jacc <- cbind(
    data.frame(rilievo = rownames(ris.Jacc)),
    data.frame(ris.Jacc)
  )
  ...
  ris.bray.TOT$Facies.Similarity <- paste(
    ris.bray.TOT$Facies,
    round(ris.bray.TOT$BrayDistance, 3),
    sep = ": "
  )
  ris.bray.TOT <- ris.bray.TOT[, c(
    "Rilievo",
    "Tipo",
    "Facies",
    "BrayDistance",
  ...
  ris.Jacc.TOT$Facies.Similarity <- paste(
    ris.Jacc.TOT$Facies,
    round(ris.Jacc.TOT$JaccardDistance, 3),
    sep = ": "
  )
  ris.Jacc.TOT <- ris.Jacc.TOT[, c(
    "Rilievo",
    "Tipo",
    "Facies",
    "JaccardDistance",
